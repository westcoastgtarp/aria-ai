...
async function audit(env,member,id,risk,trigger,incidentId){const now=new Date().toISOString();await env.DB.prepare(`INSERT INTO audit_events (id,category,event_type,actor_user_id,subject_type,subject_id,related_ticket_id,details_json,occurred_at,recorded_at) VALUES (?, 'Lifeline Safety','lifeline_human_support_escalated',?,'member',?,?,?, ?, ?)`).bind(`AUD-${crypto.randomUUID()}`,member.user_id,member.user_id,id,JSON.stringify({risk,trigger,channel:'chat_only',queueCategory:'Member Communication',incidentId:incidentId||null}),now,now).run();}
...
async function escalate(request,env){
  ...
  const risk=String(body?.risk||'').toLowerCase();const trigger=String(body?.trigger||'automatic_distress_monitor').slice(0,80);
  ...
  try{await audit(env,member,id,risk,trigger,persistence.incidentId);}catch(error){console.error('Lifeline support audit failed',error);}
...
export async function handleLifelineSupportRoute(request,env){const url=new URL(request.url);if(url.pathname==='/api/member/lifeline/support-escalate'&&request.method==='POST')return escalate(request,env);return null;}
