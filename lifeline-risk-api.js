...
async function recordSafetyAudit(env,member,classification){
  if(!env.DB||!['high','critical'].includes(classification.level))return;
...
- concern: signs of distress, fear, worry, pain, panic, confusion, or deterioration that merit supportive follow-up and closer monitoring, but do not yet justify an emergency-contact countdown.
- high: substantial safety concern or credible risk where Aria should begin a 5-minute member response window and offer the approved Care Circle contact and emergency calling.
- critical: strong indication of immediate or imminent danger, self-harm intent, overdose, inability to breathe, unconsciousness, active violence, or other urgent threat where Aria should begin a 2-minute response window and prominently offer the approved contact and emergency calling.
...
- Prefer concern over high when evidence is ambiguous. Prefer high/critical when the member clearly describes present danger.
...
async function classifyWithAI(env,message,history){
  ...
  if(!parsed)throw new Error('invalid_risk_response');
  const level=normalizeLevel(parsed.level);
  const confidence=Math.max(0,Math.min(1,Number(parsed.confidence)||0));
  return {
    level,
    confidence,
    reason:String(parsed.reason||'Conversation-aware Lifeline classification.').slice(0,240),
    responseWindowSeconds:windowFor(level),
    source:'ai'
  };
}
...
async function handleAssess(request,env){
  ...
  if(!(await hasLifelineAccess(env,member.user_id))){
    return json({ok:false,code:'lifeline_unavailable',error:'Lifeline conversation monitoring is not active for this account.'},{status:403});
  }
  ...
  if(env.AI&&typeof env.AI.run==='function'){
    try{
      classification=await classifyWithAI(env,message,history);
    }catch(error){
      console.error('Lifeline AI risk classification failed; using fallback',error);
    }
  }

  if(!classification){
    const level=fallbackRisk(`${history.map(h=>h.content).join(' ')} ${message}`);
    classification={
      level,
      confidence:level==='normal'?0.55:0.7,
      reason:'Conservative fallback classification used because the AI risk monitor was unavailable.',
      responseWindowSeconds:windowFor(level),
      source:'fallback'
    };
  }
...
export async function handleLifelineRiskRoute(request,env){
  const url=new URL(request.url);
  if(url.pathname==='/api/member/lifeline/risk'&&request.method==='POST')return handleAssess(request,env);
  return null;
}
